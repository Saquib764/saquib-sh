...
function depth_to_3d(depth_data, image_data) {
  ...
  for (let i = 0; i < image_data.width * image_data.height; i++) {
    const depth = (depth_data.data[i] - min) * 255.0 / (max - min)
    // const z = 100 * (depth - 255) / 255.0
    const z = -10.0 * 255/ (depth + 10.0)
...
onMounted(async ()=>{
  ...
  let depth_data = await get_image_data('/images/3d/bottle1_depth_large.png')
  let image_data = await get_image_data('/images/3d/bottle1.png')
  ...
  console.log(depth_u8.data[1000012])
  ...
  const orbitController = useOrbitControl()
